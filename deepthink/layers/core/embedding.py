...
class Embedding(BaseLayer):
    ...
    input_shape : tuple
        The shape of the input tensor. I.e. (batch, sequence_length).
    ...
    def forward(self, inputs):
        ...
        Parameters
        ----------
        inputs : np.array, shape (batch_size, input_length)
            Input sequence of word indices.

        Returns
        -------
        output : np.array, shape (batch_size, input_length, output_dim)
            Embedding vectors for the input sequence.
        """
        self.input = inputs
        # Get the vectors and transpose to ensure channels first
